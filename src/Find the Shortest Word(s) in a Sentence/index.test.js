...
test('findShortestWords', () => {
  ...
  expect(findShortestWords('Happiness depends upon ourselves.')).toEqual([
    'upon',
  ]);
  expect(findShortestWords('We become what we think about.')).toEqual([
    'we',
    'we',
  ]);
  expect(findShortestWords('I think the solution is fairly obvious.')).toEqual([
    'i',
  ]);
  expect(
  ...
  expect(
    findShortestWords('Believe you can and you’re halfway there.'),
  ).toEqual(['and', 'can', 'you']);
  expect(
